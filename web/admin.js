(() => {
  ...
  let machines = [];
  let routing = { strategy: "round_robin", rateLimitCooldownMs: 60 * 60 * 1000 };
  let toastTimer;
  ...
  async function load() {
    ...
    try { const [machineResult, routingResult] = await Promise.all([api("/admin/machines"), api("/admin/routing")]); machines = machineResult.data || []; routing = routingResult; $("routing-strategy").value = routing.strategy; $("routing-cooldown-minutes").value = Math.max(1, Math.round(routing.rateLimitCooldownMs / 60000)); render(); setConnection(true, "已连接"); $("last-updated").textContent = `最后更新 ${new Date().toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })}`; $("auth-modal").classList.add("hidden"); }
